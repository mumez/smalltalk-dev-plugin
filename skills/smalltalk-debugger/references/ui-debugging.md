...
Comprehensive guide for diagnosing hidden debugger windows in a Smalltalk (Pharo/Squeak) image using the `read_screen` MCP tool.

## Purpose

When developing with a Smalltalk image through MCP tools, operations can suddenly stop responding or timeout. This often indicates that a **debugger window has opened in the Smalltalk image** due to an error or exception. Since the debugger UI is not visible from the AI editor, this creates an invisible blocking state where:

- MCP tool calls hang indefinitely
- Operations timeout without clear error messages
- The Smalltalk image appears unresponsive to remote commands
- No feedback reaches the development environment

The `read_screen` tool provides visibility into the Smalltalk image's UI state, allowing you to detect and diagnose these hidden debugger windows.

## The read_screen Tool

### Overview

The `read_screen` tool captures screenshots and extracts UI structure from the running Smalltalk image. It provides detailed information about all visible UI components, including windows, morphs, and their hierarchical relationships.

### Parameters

**target_type** (string, default: 'world')
- `'world'`: Captures all morphs in the Morphic World (display environment) — works on both Pharo and Squeak
...
- `'spec'` (**Pharo only**): Examines Spec presenter windows
...
- `'roassal'` (**Pharo only**): Inspects Roassal visualization canvases
...
> **Docker note**: When the Smalltalk image runs inside Docker (check by `docker ps`), screenshot files are saved to `/root/screenshots` inside the container, not to `/tmp` on the host. Check `compose.yml` `volumes:` to find where `/root/screenshots` is mounted on the host side before looking for screenshot files.
...
Alternatively, on Pharo, use 'spec' target to focus on windows:
...
**Message Template:**
```
I've detected that a debugger window has opened in your Smalltalk image.
...
Please:
1. Switch to your Smalltalk image
...
manual intervention in the Smalltalk image is required.
```
...
- The Smalltalk image becomes unresponsive to commands
...
- Verifying the Smalltalk image's UI state after complex operations
- Checking if modal dialogs are blocking
- Confirming window layouts during UI development
- Debugging Spec or Roassal visualization issues (Pharo only)
...
If you need to locate a saved screenshot file and the Smalltalk image is running in Docker, check `compose.yml` for the host-side mount of `/root/screenshots` (it is **not** necessarily `/tmp` on the host).
...
## UI Debugging of Spec2 (Pharo only)

Use `read_screen` to verify Spec2 presenter UIs during development. Spec2 is Pharo-specific; this section does not apply to Squeak.
...
The `read_screen` tool is essential for diagnosing invisible blocking states in Smalltalk development through MCP. When operations hang:
...
Remember: **The Smalltalk debugger requires manual intervention**. You cannot close it remotely through MCP tools. Always inform the user and request they handle it in the Smalltalk image.
